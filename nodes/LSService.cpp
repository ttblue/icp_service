...
Matrix4f fitLSTransform(ColorCloudPtr cloud1, ColorCloudPtr cloud2,
  ...
  ColorCloudPtr cloud2_ds = cloud2;//downsampleCloud(cloud2, sz);
  ...
  while (iter <= MAX_ITER) {
    ...
    float distTol = max(0.5 -  4*iter/30, 0.05);
...
bool findLSTransform(icp_service::LSTransform::Request &req, 
  ...
  pcl::fromPCLPointCloud2(cloud1_PCLPC2, *cloud1);
  pcl::fromPCLPointCloud2(cloud2_PCLPC2, *cloud2);
